...
\end{example}
    ...
    \begin{center}
	    \item 1) $r_{BornInCity} \circ r_{CityInProvince} \sqsubseteq r_{BornInProvince}$
        \item 2) $r_{BornInProvince} \circ r_{ProvinceInCountry} \sqsubseteq r_{Nationality}$
        \item 4) $r_{BornInCity}(Mike, Hangzhou)$
        \item 5) $r_{CityInProvince}(Beihai, Guangxi)$
		\item 6) $r_{ProvinceInCountry}(Zhejiang, China)$
    \end{center}
    ...
    对决断集模板进行二分划分$(\mathcal{J}_1, \mathcal{J}_2)$，使得$\mathcal{J}_1 \cap \mathcal{J}_2 = \emptyset$且$\mathcal{J}_1 \cup \mathcal{J}_2 = \mathcal{J}_p$。
		...
		\end{center}
		根据划分2)，可以得到一个$\theta$使得 $\mathcal{J}_1\theta \subseteq \mathcal{O}$，其中$\theta = \{A \mapsto Mike, C \mapsto Hangzhou, B \mapsto China\}$，所以我们设$\mathcal{E}_1 = \mathcal{J}_2\theta = \{r_{CityInProvince}(Hangzhou, e_1), r_{ProvinceInCountry}(e_1, China)\}$，又因为$\mathcal{O} \cup \mathcal{J}_2\theta$是一致的，$\mathcal{O} \cup \mathcal{J}_2\theta \models \alpha$，且对于所有的$\mathcal{E}' \in subs_1(\mathcal{E}_1) \cup lifts_1(\mathcal{E}_1)$都有$\mathcal{O} \cup \mathcal{E}' \not \models \alpha$，所以$\mathcal{E}_1$是一个合理的解释。
		
		\begin{center}
	    \item 3)\ $J_1$= $\{r_{BornInCity}(A, C), r_{ProvinceInCountry}(D, B)\}$, $J_2$=$\{r_{CityInProvince}(C, D)\}$
		\end{center}
		根据划分3)，可以得到一个$\theta$使得 $\mathcal{J}_1\theta \subseteq \mathcal{O}$，其中$\theta = \{A \mapsto Mike, C \mapsto Hangzhou, B \mapsto China, D \mapsto Zhejiang\}$，所以我们设$\mathcal{E}_2 = \mathcal{J}_2\theta = \{r_{CityInProvince}(Hangzhou, Zhejiang)\}$，又因为$\mathcal{O} \cup \mathcal{J}_2\theta$是一致的，$\mathcal{O} \cup \mathcal{J}_2\theta \models \alpha$，且对于所有的$\mathcal{E}' \in subs_1(\mathcal{E}_2) \cup lifts_1(\mathcal{E}_2)$都有$\mathcal{O} \cup \mathcal{E}' \not \models \alpha$，所以$\mathcal{E}_2$是一个合理的解释。
		\begin{center}
