...
在这一章中，我们将介绍逻辑基础知识，并给出后述章节将使用的逻辑一些性质和已有结果。
第一节介绍经典逻辑，包括一阶逻辑和二阶逻辑；
第二、三节分别介绍本文主要研究的两种非单调语言：一阶限定理论限定语义下的理论和一阶稳定模型语义下的理论；
最后一节介绍回答集程序以及它的一些基本性质。


\section{经典一阶逻辑}

与命题逻辑仅处理简单的陈述命题不同，一阶逻辑还额外包含了谓词、函词和量词。
一阶语言$L$是一阶逻辑使用的形式语言，一般由非逻辑符号和逻辑符号组成。非逻辑符号包括：{\kai 谓词符号}，{\kai 函词符号}。逻辑符号包括：{\kai 个体变元符号}，{\kai 量词符号}$\forall$ 和$\exists$，{\kai 逻辑联结词}$\neg$、$\wedge$、$\vee$、$\rightarrow$、$\leftrightarrow$，左括号，右括号和逗号。在本文中还使用两个特别的符号$\top$和$\bot$，分别表示“真”和“假”。

在讨论一阶逻辑的时候，一阶语言通常会与一个符号集联系起来。在本文中，一个{\kai 符号集}(Vocabulary)是由若干个谓词常元符号和函词常元符号构成的一个集合。
谓词常元符号与函词常元符号统一称为{\kai 常元}。
每一个常元均对应一个自然数，称为该常元符号的{\kai 元数}。
通常地，元数为0的谓词常元称为{\kai 命题常元}，元数为0的函词常元称为{\kai 个体常元}。
下面定义关于符号集3的项、原子公式、一阶公式和一阶语句。
\begin{definition}
符号集$v$的{\kai 项}（记为$L^{v}$-项）定义如下:
\begin{enumerate}
  \item 符号集$v$中的个体常元符号和个体变元符号是$L^{v}$-项；
  \item 若$t_1,t_2,...,t_n$是$L^{v}$-项，并且$f$是符号集$v$中的$n$元函词常元符号，则$f(t_1,t_2,...,$~$t_n)$也是$L^{v}$-项；
  \item 所有的$L^{v}$-项都是有限次使用1和2得到的。
\end{enumerate}
\end{definition}

\begin{definition}
任意给定一个符号集$v$，若$t_1,t_2,...,t_n$是$L^{v}$-项，且$P$是符号集$v$中的$n$元谓词常元符号，则$P(t_1,t_2,...,t_n)$是$v$的{\kai 原子公式}（记为$L^{v}$-原子公式）。
\end{definition}

\begin{definition}
符号集$v$的{\kai 公式}（记为$L^{v}$-公式）定义如下:
\begin{enumerate}
  \item $L^{v}$-原子公式是$L^{v}$-公式；
  \item 若$\varphi$是$L^{v}$-公式，则($\neg \varphi$)也是$L^{v}$-公式；
  \item 若$\varphi_1$，$\varphi_2$是$L^{v}$-公式，则($\varphi_1 * \varphi_2$)也是$L^{v}$-公式，$*$是$\wedge$、$\vee$、$\rightarrow$、$\leftrightarrow$中的一个；
  \item 若$\varphi(u)$是$L^{v}$-公式，$u$是$\varphi$中出现的个体变元符号，并且$x$不在$\varphi$中出现过，则$\exists x \varphi(x)$，$\forall x \varphi(x)$也是$L^{v}$-公式；
  \item 所有的$L^{v}$-公式都是有限次使用1~4得到的。
\end{enumerate}
\end{definition}

\begin{definition}
若$L^{v}$-公式中的每一个个体变元符号都处于某个量词的辖域下。则该公式称为$L^{v}$-{\kai 一阶语句}。
\end{definition}

所有$L^{v}$-公式组成的集合称为符号集$v$的一阶语言，记为$L^{v}$。
给定一个一阶语句$\varphi$，定义$\varphi$的{\kai 符号集}为由出现在$\varphi$中的所有谓词常元和函词常元的集合，记为$v(\varphi)$。

{\kai 理论}是由若干个语句组成的集合，在本文中，一阶理论可被看作由集合中的一阶语句合取而成的一阶语句。

范式是具有标准形式的公式，公式可以经过变换成为范式。在逻辑语言中，有两个重要的范式分别为{\kai 合取范式}（Conjunction Normal Form，简写为CNF）和{\kai 析取范式}（Disjunction Normal Form，简写为DNF）。合取范式是以析取子式为合取项的合取式，形如$(A_{11}~\vee~...~\vee~A_{1n_1})~\wedge~...~\wedge~(A_{i1}~\vee~...~\vee~A_{in_j})$，
析取范式是以合取子式为析取项的析取式，形如$(A_{11}~\wedge~...~\wedge~A_{1n_1})~\vee~...~\vee~(A_{i1}~\wedge~...~\wedge~A_{in_j})$。

我们称任一形如$Q_1x_1Q_2x_2\cdot\cdot\cdot Q_nx_n\varphi$的一阶公式为一个{\kai 前束范式}（Prenex Normal Form，简写为PNF），其中$Q_1,...,Q_n$ 是$\forall$或$\exists$，并且$\varphi$中不包含量词。
在经典一阶逻辑中，每个公式都可以变换为与之等价的前束范式。
在本文中，为书写方便，通常将连续相同的量词简写成一个量词块，即将形如$Qx_1Qx_2\cdot\cdot\cdot Qx_n\varphi$的一阶公式缩写为$Q\bar{x}\varphi$，称前缀$Q\bar{x}$为一个量词块，
其中$Q$为量词$\forall$或$\exists$，$\bar{x}$表示上述个体变元的序列$x_1x_2...x_n$。
若一个前缀范式$\psi$可被按上述方式缩写为如下形式：$Q_1\bar{x}_1Q_2\bar{x}_2\cdot\cdot\cdot Q_n\bar{x}_n\varphi$，
并且$\varphi$中无量词出现，每个量词块交替出现，
则称$\psi$为一个{\kai $\Sigma^0_n$-公式}当且仅当$Q_1$为存在量词$\exists$，
称称$\psi$为一个{\kai $\Pi^0_n$-公式}当且仅当$Q_1$为存在量词$\forall$。
若$\psi$为一个一阶语句，则分别称为{\kai $\Sigma^0_n$-语句}，{\kai $\Pi^0_n$-语句}。
特别地，我们称$\Pi^0_1$-公式为{\kai 全称一阶公式}，称$\Pi^0_1$-语句为{\kai 全称一阶语句}。

关于某一公式的自由个体变元的定义与标准方式相同，即存在该变元在给定公式中的某一出现不在任一量词的辖域之中。
若$\bar{x}$是由一阶公式$\varphi$中所有自由个体变元
所构成的序列，则称公式$\forall \bar{x}\varphi$为$\varphi$的{\kai 全称闭包}，简记为$\bar{\forall}\varphi$。

我们称一个原子公式为{\kai 正文字}，称其否定式为{\kai 负文字}，正文字和负文字统称为{\kai 文字}(Literal)。
我们定义一阶公式的{\kai 否定范式}（Negative Normal Form，简写为NNF）仅由文字、$\wedge$、$\vee$、$\forall$和$\exists$构成。
在经典一阶逻辑中，每个公式都可以变换为与之等价的否定范式。


下面给出一阶逻辑的语义。
\begin{definition}
定义一个符号集为$v$的{\kai 结构}（简称为$v$-结构）$\mu$是具有如下属性的二元组：
\begin{enumerate}
  \item $A$是一个非空集，称为$\mu$的{\kai 论域}；
  \item 从符号集$v$到论域$A$上所有关系和函数的映射，且满足：
  \begin{enumerate}
    \item[(1)] 对于$v$上的任意$n$元谓词常元$P$，$\mu(P)$是$A$上的$n$元关系，称之为$\mu$对$P$的解释；
    \item[(2)] 对于$v$上的任意$n$元函数常元$f$，$\mu(f)$是$A$上的$n$元全函数，称之为$\mu$对$f$的解释。
  \end{enumerate}
\end{enumerate}
\end{definition}

一个$v$-结构$\mu$是有穷的（或可数的）当且仅当其论域$A$是有穷的（或可数的）。

设$v$为任意一个符号集，若$v$-结构$\mu$和$v$-结构$\nu$满足：
\begin{itemize}
  \item $\nu$的论域$B$是$\mu$的论域的子集；
  \item 对$v$中每一k元谓词常元$P$和论域$B$中每一k元组$\bar{a}$有$\bar{a} \in \mu(P)$当且仅当$\bar{a} \in \nu(P)$；
  \item 对$v$中每一k元函词常元$f$和论域$B$中每一k元组$\bar{a}$有$\mu(f)(\bar{a})=\nu(f)(\bar{a})$。
\end{itemize}
则称结构$\nu$是$\mu$的{\kai 子结构}，或者结构$\mu$是$\nu$的{\kai 扩张}，记为$\nu\subseteq\mu$。

设$\sigma$和$\tau$为任意两个个符号集，且$\sigma \subseteq\tau$，若$\sigma$-结构$\mu$和$\tau$-结构$\nu$满足：
\begin{itemize}
  \item $\nu$和$\mu$具有相同的论域；
  \item 对$\sigma$中每一k元谓词常元$P$和论域中每一k元组$\bar{a}$有$\bar{a}\in\mu(P)$当且仅当$\bar{a} \in \nu(P)$；
  \item 对$\sigma$中每一k元函词常元$f$和论域中每一k元组$\bar{a}$有$\mu(f)(\bar{a})=\nu(f)(\bar{a})$。
\end{itemize}
则称结构$\mu$为结构$\nu$在符号集$\sigma$上的{\kai 限制}（记为$\nu|\sigma$），称结构$\nu$是$\mu$关于符号集$\tau$的一个{\kai 膨胀}。

结构实际上将一阶公式中的所有常元符号都与一个论域联系起来，而除了常元一阶公式中还包括变元符号，我们还需要对变元符号进行赋值。
结构$\mu$上的一个{\kai 赋值}$\alpha$也是一个映射，它将出现在一阶公式中的每个自由个体变元映射为论域$A$内的元素。

设$\varphi$是一个$L^v$-公式，$\alpha$是结构$\mu$上的一个赋值，
则$\mu$与$\alpha$关于$\varphi$的满足关系按照标准递归定义。
若$\alpha$在结构$\mu$上满足的公式$\varphi$，
则记为$(\mu,\alpha)\models\varphi$。
特别地，如果$\varphi$是一个$L^v$-一阶语句，即没有自由变元，则赋值$\alpha$可以省去，记为$\mu\models\varphi$，称结构$\mu$满足语句$\varphi$，
或者结构$\mu$是语句$\varphi$的一个{\kai 模型}（Model）。
设$\Gamma$是一个语句集，若结构$\mu$满足$\Gamma$中的所有语句，则称结构结构$\mu$满足语句集$\Gamma$，或结构$\mu$是语句集$\Gamma$的一个模型。


设$\varphi$和$\psi$是两个一阶公式，$\Sigma$是一类结构。$\Sigma$
若对于任何$\Sigma$中的结构$\mu$及$\mu$上的赋值$\alpha$均有$(\mu,\alpha)\models\varphi$当且仅当$(\mu,\alpha)\models\psi$，
则我们称公式$\varphi$在$\Sigma$上逻辑等价于公式$\psi$，或者公式$\varphi$与公式$\psi$在$\Sigma$上是{\kai 逻辑等价}的，记为$\varphi \equiv_{\Sigma} \psi$。
若$\Sigma$包含所有结构时，则称公式$\varphi$逻辑等价于公式$\psi$，或者公式$\varphi$与公式$\psi$是逻辑等价的，记为$\varphi \equiv \psi$。
若$\Sigma$仅包含所有有穷结构时，则记为$\varphi \equiv_{fin} \psi$。


\section{二阶逻辑}

二阶逻辑$L_{II}$是一阶逻辑的一个扩充。
二阶逻辑$L_{II}$的语言使用和一阶语言相同的符号表。
与一阶逻辑不同的是，一阶逻辑仅允许个体变元，在二阶逻辑中增加了一可数集的
谓词变元和一可数集的函词变元，每一谓词变元和每一函词变元均有一相关的
非负整数，称为其{\kai 元数}。特别地，0元的谓词变元又被称为命题变元，而0元的函词变元
实质上就是个体变元。因此，我们不妨将函词变元的元数限制为正整数。

符号集为$v$的二阶项（记为$v$-二阶项）是在一阶定义中允许函词变元后得到的项，而其构成规则与函词常元的构成规则完全相同。
符号集为$v$的二阶公式（记为$L_{II}^{v}$-公式）
是封闭与所有$L^{v}$-公式的构造规则（需将原构造规则的$v$-项替换成$v$-二阶项）以及下述三个规则的最小集：
\begin{enumerate}
